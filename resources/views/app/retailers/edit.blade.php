      ...
      <div class="row">
         ...
         <div class="col-sm-4">
            <div class="left-card">
               ...
               skriptz.des = function () {
                  $('button[data-crawl="button"]').click(function(){
                     $(this).text('Fetching...Be Patient!');
                     var query = 'select * from html where url="' + $('input[data-crawl="input"]').val() + '" and xpath="*"';
                     var url = 'https://query.yahooapis.com/v1/public/yql?q=' + encodeURIComponent(query);
                     var regex = /(?:\:)\b(\w*)\b(?=\:)/g

                     var jqxhr =  $.get(url, function(data) {

                        var html = $(data).find('head')
                        var meta = html.find('meta[name=description]').attr('content');

                        if (meta) {
                           $('textarea[data-crawl="description"]').val(meta);
                           $('button[data-crawl="button"]').text('Success!');
                        } else {
                           $('button[data-crawl="button"]').text('No Result!, Try another?');
                           $('textarea[data-crawl="description"]').attr("placeholder", "Uh damn! The website may have denied our crawl, as a result we could not fetch the Meta Description from the provided website.");
                        }
                     })
                     .fail(function() {
                        $('button[data-crawl="button"]').text('Yahoo API Failed');                     });
                  });
               }

...
class ImageRepository
{
...

    {
        ...
        $nameWithoutExt = substr($name, 0, strlen($name) - strlen($extension) - 1);

        $filename = $this->sanitize($nameWithoutExt);
        $allowed_filename = $this->createUniqueFilename( $filename, $extension );

        $original = $this->original( $photo, $allowed_filename );
        $icon = $this->icon( $photo, $allowed_filename );
        ...
        $insert->filename       = $allowed_filename;
        $insert->original_name  = $name;
        $insert->save();
    ...
    {
        $manager = new ImageManager();
        $image = $manager->make( $photo )->resize(300, null, function ($constraint) {
